"""Three-Chain DNA-Mamba3: 基于 Mamba3 (梯形离散化 + RoPE) 的三链重构

A 版本 ThreeChainMamba3: 与 ThreeChainMamba2 同骨架，只把 Mamba2 → Mamba3
    用于对照实验: 隔离 "Mamba3 (梯形+RoPE) vs Mamba2 (SSD)" 的纯效果。

与 three_chain_mamba2.py 的差异:
    1. import Mamba3 from .mamba3_ref (无 Triton, Python 顺序扫描)
    2. make_mamba3 去掉 d_conv 参数 (Mamba3 用梯形离散化代替卷积)
    3. BidirectionalMamba3 同向包装 (Mamba3 forward I/O 与 Mamba2 一致)
    4. HeteroMamba3/ThreeChainMamba3 类名替换
    5. 保留 enable_m3/enable_jepa/save_m3 接口 (对照实验时默认关闭)
...
Mamba3 三链参数表 (与 Mamba2 版对齐, 仅去 d_conv):
    ...
    注意: Mamba3 无 causal_conv1d, 空间链 headdim 不再强制 32 (但保留以对照公平)

速度: Mamba3 forward ≈ 30x 慢于 Mamba2 (Python for loop vs Triton)
      100 步训练约 2-3 分钟 (可接受), 长训练需评估
"""
import math
...
def make_mamba3(d_model, d_state, expand, headdim):
    """构造 Mamba3 (梯形离散化 + RoPE)。参数约束:
        - d_inner = expand * d_model 必须被 headdim 整除
        - 无 d_conv 参数 (Mamba3 用梯形离散化代替卷积)
        - 无 causal_conv1d 依赖 (不会触发 stride 对齐错误)
    """
    return Mamba3(
        d_model=d_model,
        d_state=d_state,
...
def _ckpt_call(mamba, x):
    """梯度检查点包装 Mamba3 调用。

    必要性: Mamba3 reference 是 Python for-loop, autograd 会为 L 步的每个操作
    保存中间张量, 单步 backward 内存 = O(L × 张量数)。实测 N=6 T=30 小输入
    单步就吃 9.6GB RSS; N=8 T=100 会 OOM (16GB 机器扛不住)。

    checkpoint 让 forward 不保存中间值, backward 时重算一遍 forward。
    内存从 O(L) 降到 O(1) (只存输入), 代价是 backward 慢 ~2x (重跑 loop)。

    eval 时 (torch.no_grad 上下文) 自动直通, 不影响推理速度。
    """
...
class BidirectionalMamba3(nn.Module):
    ...
    def __init__(self, d_model, d_state, expand, headdim):
        super().__init__()
        self.mamba = make_mamba3(d_model, d_state, expand, headdim)

    def forward(self, x):
        # x: (B, L, d) — 用 checkpoint 包装避免 backward 内存爆炸
        y_fwd = _ckpt_call(self.mamba, x)
        y_bwd = _ckpt_call(self.mamba, torch.flip(x, dims=[1]))
...
class HeteroMamba3(nn.Module):
    ...
    与 HeteroMamba2 完全同构, 仅 Mamba2 → Mamba3。
...
class ThreeChainMamba3(nn.Module):
    """A 版本: 统一时空张量三链 Mamba3。

    与 ThreeChainMamba2 完全同构, 仅 Mamba2 → Mamba3。
    用于对照实验: 隔离 Mamba3 (梯形离散化 + RoPE) vs Mamba2 (SSD) 的纯效果。
